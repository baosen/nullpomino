...
/**
 * Drives {@link LineRaceMode#onSetting} across every numeric configuration
 ...
 * exercises the BUTTON_E (x100) and BUTTON_F (x1000) speed multipliers.
 */
class LineRaceModeSettingWraparoundTest {
	...
	private static void change(LineRaceMode mode, GameEngine engine, int cursor, int dirButton, int multButton) throws Exception {
	...
	@Test
	void bgmGoaltypePresetAndBigWrapAtBothBounds() throws Exception {
		LineRaceMode mode = new LineRaceMode();
		GameEngine engine = freshEngine(mode);
		mode.playerInit(engine, 0);
		...
		// case 8: big toggles (also fills in the unexercised switch case)
