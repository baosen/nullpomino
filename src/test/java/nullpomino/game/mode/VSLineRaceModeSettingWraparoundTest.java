...
/**
 * Drives {@link VSLineRaceMode#onSetting} across every numeric configuration
 * cursor, pushing each value past both bounds so the wraparound branches all
 * fire. Also exercises the BUTTON_E (x100) / BUTTON_F (x1000) multipliers.
 */
class VSLineRaceModeSettingWraparoundTest {
	...
	private static void change(VSLineRaceMode mode, GameEngine engine, int cursor, int dir, int mult) throws Exception {
	...
	@Test
	void presetGoalBgmAndToggleCursorsWrap() throws Exception {
		VSLineRaceMode mode = new VSLineRaceMode();
		GameEngine engine = freshEngine(mode);
		mode.playerInit(engine, 0);
