...
/**
 * Covers the private ranking helpers in {@link DigChallengeMode}: checkRanking
 * (score-primary/lines-secondary/time-tertiary with higher time being better)
 * and updateRanking.
 */
class DigChallengeModeRankingTest {

	// -----------------------------------------------------------------------
	// checkRanking — score primary, lines secondary, higher time is better
	// -----------------------------------------------------------------------
	...
	@Test
	void checkRankingUsesHigherTimeAsTertiaryKey() throws Exception {
		DigChallengeMode mode = new DigChallengeMode();
		freshEngine(mode);
		...
		// same score and lines, MORE remaining time (higher value) is better
		int rank = invokeCheckRanking(mode, 500, 10, 5001, 0);
		assertEquals(0, rank);
	}
	...
	private static int invokeCheckRanking(DigChallengeMode mode, int sc, int li, int time, int type)
			throws Exception {
	...
	private static void invokeUpdateRanking(DigChallengeMode mode, int sc, int li, int time, int type)
			throws Exception {
		Method m = DigChallengeMode.class.getDeclaredMethod(
