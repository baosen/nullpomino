...
class AIRemainingCoverageTest {
    ...
    @Test void nohoho_pieceHoldNull() throws Exception { Nohoho a = new Nohoho(); engine.aiUseThread = false; a.init(engine,0); engine.createFieldIfNeeded(); engine.nowPieceObject = new Piece(Piece.PIECE_T); engine.nowPieceObject.applyOffsetArray(engine.ruleopt.pieceOffsetX[Piece.PIECE_T],engine.ruleopt.pieceOffsetY[Piece.PIECE_T]); engine.nowPieceX = 4; engine.nowPieceY = 18; engine.holdPieceObject = null; engine.nextPieceCount = 0; engine.nextPieceArraySize = 3; java.lang.reflect.Field nf = GameEngine.class.getDeclaredField("nextPiece"); nf.setAccessible(true); nf.set(engine, new Piece[]{new Piece(Piece.PIECE_T),new Piece(Piece.PIECE_S),new Piece(Piece.PIECE_Z)}); a.thinkBestPosition(engine,0); }
    ...
    @Test void basicAI_thinkMainNoPlace() { BasicAI a = new BasicAI(); Field f = new Field(10,20,0,false); for (int y = 0; y < 20; y++) for (int x = 0; x < 10; x++) f.setBlockColor(x,y,1); assertEquals(0, a.thinkMain(engine,0,0,0,-1,f,new Piece(Piece.PIECE_T),null,null,1)); }
