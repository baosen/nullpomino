...
/**
 * Tests for the v2 (mesh session) LAN announce format in {@link NetLanDiscovery}:
 * round trip, coexistence with v1, rejection by old clients, and session dedupe.
 */
class NetLanDiscoveryMeshTest {

    @Test
    void meshAnnounceRoundTrip() {
        byte[] data = NetLanDiscovery.encodeMeshAnnounce(
    ...
    void oldClientRejectionIsPinnedByVersionField() {
        ...
        byte[] data = NetLanDiscovery.encodeMeshAnnounce(9202, "Hoster", "abc123", "Lobby", 1);
        String[] parts = new String(data, StandardCharsets.UTF_8).split("\t");
        assertEquals("2", parts[1]);
    }

    @Test
    void malformedMeshAnnouncesAreRejected() {
    ...
    void dedupeCollapsesSameSessionToLowestHostPort() {
        ...
        in.add(mesh("192.168.1.20", 9202, "B", "sessA", 3));
        in.add(mesh("192.168.1.10", 9202, "A", "sessA", 3));
        in.add(mesh("192.168.1.30", 9202, "C", "sessB", 2));
        ...
        assertFalse(out.get(2).mesh);
    }

    private static NetLanDiscovery.Announce mesh(String addr, int port, String name,
